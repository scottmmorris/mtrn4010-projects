...
function z5165456Project2MTRN4010()

    % Define global variables to handle the plotting of Local (lhand) and Global
    % coordinates (ghand)
    global lhand lhand2 lhand3 ghand ghand2 ghand4 ghand5 ghand6 ghand7 ghand8 ghand9 ghand10 ghand11 ghand12 ghand13 ghand14 Landmarks;
    ...
    for i=i0:L
        ...
        % heading (X(3))
        X(:, i+1) = X(:, i) + dt * [NewSpeed * cos(deg2rad(X(3, i)));
                                    NewSpeed * sin(deg2rad(X(3, i)));
                                    NewAngRate];
...
function ProcessLiDAR( r, I , X )
    ...
    if (ooi(1,1) ~= 0 && ooi (1,2) ~= 0)
        ...
        ooiyc(:,2) = ooi(:,2) + 0.46;
        ooixg = ooi(:,1) * cos(deg2rad(X(3) - 90)) - ooiyc(:,2) * sin(deg2rad(X(3) - 90)) + X(1);
        ooiyg = ooi(:,1) * sin(deg2rad(X(3) - 90)) + ooiyc(:,2) * cos(deg2rad(X(3) - 90)) + X(2);
